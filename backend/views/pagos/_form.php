...
<div class="documento-form">
    ...
    <?php $form = ActiveForm::begin(['id' => 'dynamic-form']); ?>
 	<div class="row">
 		<div class="col-sm-6">     
    	<!-- <?= $form->field($model, 'idCliProCia')->dropDownList(ArrayHelper::map(Cliprocia::find()->all(),'id','cliProCiaEntidad'),['prompt'=>'Seleccione Cliente']); ?>   -->	
    		<?= $form->field($model, 'idCliProCia')->widget(Select2::classname(), 
    				[
    					'language' => 'es',
    					'data' => ArrayHelper::map(Cliprocia::find()->andWhere(['idCompania'=>Yii::$app->session['idCia']])->all(),'id','cliProCiaEntidad'),
 		...
 		<div class="col-sm-3">     
    		<?= $form->field($model, 'idTipoDocumento')->dropDownList(ArrayHelper::map(Tipodocumento::find()->where('idDebiCredi=2')->all(),'id','descripcion'),
...
<!-- 		
 ...
 -->
    ...
    <div class="form-group">
        <?= Html::submitButton($model->isNewRecord ? 'Agregar Pago' : 'Actualizar', ['class' => $model->isNewRecord ? 'btn btn-success' : 'btn btn-primary']) ?>
    </div>
