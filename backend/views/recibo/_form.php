    ...
    <?php $form = ActiveForm::begin(['id' => 'dynamic-form']); ?>
 	<div class="row">
    	...
    	<!-- <?= $form->field($model, 'idCliProCia')->dropDownList(ArrayHelper::map(Cliprocia::find()->all(),'id','nombre'),['prompt'=>'Seleccione Cliente']); ?>   -->	
 		...
 		<div class="col-sm-3">     
    		<?= $form->field($model, 'idTipoDocumento')->dropDownList(ArrayHelper::map(Tipodocumento::find()->where('id=2')->all(),'id','descripcion'),['readonly'=>true]); ?>
    	</div>
		<div class="col-sm-3">     
...
<!-- 		
 ...
 -->
    ...
    <div class="form-group">
        <?= Html::submitButton($model->isNewRecord ? 'Insertar' : 'Actualizar', ['class' => $model->isNewRecord ? 'btn btn-success' : 'btn btn-primary']) ?>
    </div>
